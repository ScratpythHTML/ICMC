...
const WestwayCrashpadsComponent = () => {
  const navigation = useNavigation<NavigationProp<RootStackParamList>>();
  const { data: crashpads } = useGetCrashpads(StorageLocation.Westway);
  const handleOnPress = (item: CrashpadDto) => {
    navigation.navigate('WestwayCrashpadComponent', { id: item.id });
  };
...
export default WestwayCrashpadsComponent;
