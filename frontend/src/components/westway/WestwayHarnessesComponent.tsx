...
const WestwayHarnessesComponent = () => {
  const navigation = useNavigation<NavigationProp<RootStackParamList>>();
  const { data: harnesses } = useGetHarnesses(StorageLocation.Westway);
  const handleOnPress = (item: HarnessDto) => {
    navigation.navigate('WestwayHarnessComponent', { id: item.id });
  };
...
export default WestwayHarnessesComponent;
