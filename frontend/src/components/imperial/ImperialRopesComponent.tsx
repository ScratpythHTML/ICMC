...
const ImperialRopesComponent = () => {
  const navigation = useNavigation<NavigationProp<RootStackParamList>>();
  const { data: ropes } = useGetRopes(StorageLocation.Imperial);
  const handleOnPress = (item: RopeDto) => {
    navigation.navigate('ImperialRopeComponent', { id: item.id });
  };
...
export default ImperialRopesComponent;
